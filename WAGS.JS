...
const TARGET_SITE = "West";
const TARGET_GROUP = "Age Group Select";
...
function buildRosterFromRows(rows) {
  ...
  for (const r of rows) {
    ...
    const aaStr = normalize(r[COL.aa]);

    const timeSec = timeToSeconds(timeStr);
    if (!event || timeSec === null) continue;

    /* ----- AA qualification: time <= AA (M) ----- */
    const aaSec = timeToSeconds(aaStr);
    if (aaSec !== null && aaSec > 0 && timeSec <= aaSec) {
      addUniqueItem(obj.aaItems, obj.aaSeen, event, timeStr, timeSec);
    }
...
function render(rows) {
  ...
  rows.forEach((r) => {
    ...
    const aaHTML = listLines(r.aaItems, "No AA Times");
